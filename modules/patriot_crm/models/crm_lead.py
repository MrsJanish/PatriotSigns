...
class CrmLead(models.Model):
    ...
    # NOTE: Time tracking fields (bidding_hours, time_punch_ids, etc.) are defined
    # in patriot_timeclock/models/crm_lead.py because ps.time.punch is in that module
    ...
    def write(self, vals):
        ...
        Also syncs key fields to linked project.
        """
        res = super(CrmLead, self).write(vals)
        
        # Sync name/alias to linked project if changed
        sync_fields = {'name', 'project_alias'}
        if sync_fields & set(vals.keys()):
            for lead in self:
                # linked_project_id is defined in patriot_timeclock/models/crm_lead.py
                if hasattr(lead, 'linked_project_id') and lead.linked_project_id:
                    update_vals = {}
                    if 'name' in vals:
                    ...
                    if update_vals:
                        lead.linked_project_id.with_context(skip_sync=True).write(update_vals)
